...
setwd("D:\\data\\Meta_analysis\\Ecosystem network\\R_code_demo")
seed=25647
set.seed(seed)
...
#########################################################
########Bar plot strength list all systems (Fig. S3 & S4)
...
# Loop weights of pairwise feedback
win.graph(35,60)
par(mfcol=c(4,1),mar=c(4,4,1,1))
caca=c("orange","Yellow",'red',"black","grey50")
barmat=t(as.matrix(pfd[,-1]));colnames(barmat)=sys
ph1=barplot(barmat[,1:10],beside=T,col=caca,ylim=c(0,1.1),xlab="",ylab="Loop weight", main='Pairwise feedback')
abline(h=0)
legend('topright',rownames(barmat),lty=1,col=caca,cex=0.75,lwd=5,bty='n')
ph2=barplot(barmat[,11:19],beside=T,col=caca,ylim=c(0,1.1),xlab="System",ylab="Loop weight")
abline(h=0)

# Loop weights of triangular feedback
caca=c("orange","Yellow",'red',"black")
barmat=t(as.matrix(trifd[,-1]));colnames(barmat)=sys
ph1=barplot(barmat[,1:10],beside=T,col=caca,ylim=c(0,1.1),xlab="",ylab="Loop weight", main='Triangular feedback')
abline(h=0)
legend('topright',rownames(barmat),lty=1,col=caca,cex=0.75,lwd=5,bty='n')
ph2=barplot(barmat[,11:19],beside=T,col=caca,ylim=c(0,1.1),xlab="System",ylab="Loop weight")
abline(h=0)
